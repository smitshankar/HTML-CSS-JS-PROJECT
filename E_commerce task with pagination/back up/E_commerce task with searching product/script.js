...
function incrementValue(u) {
    ...
    if (number >= prodcutQuantity[u -1] || (JSON.parse(localStorage.getItem(`productquantity${u}`)) >= prodcutQuantity[u])) {
...
function incrementValuePannel(u) {
    ...
    let prodcutQuantity = JSON.parse(localStorage.getItem("prodcutQuantity"))
    let p = document.getElementById(`cardvaluepannel${u}`).innerHTML;
    if (p >= prodcutQuantity[u - 1] || (JSON.parse(localStorage.getItem(`productquantity${u}`)) >= prodcutQuantity[u])) {
...
function searchProductIncrement(productId) {
    ...
    if (search_quantity >= prodcutQuantity[productId - 1] || (JSON.parse(localStorage.getItem(`productquantity${productId}`)) >= prodcutQuantity[productId])) {
        alert(`${y.title} sold out from`);
        event.preventDefault();
    }
    else {
