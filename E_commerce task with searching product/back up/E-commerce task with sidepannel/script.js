                                                                        ...
                                                                        <div class="sectiondiv">
                                                                            <div class="container flex">
                                                                                ...
                                                                                <div class="searchtext width1 flex-search">
                                                                                     ...
                                                                                     <input type="text" placeholder="search..." class="search-text" id="search-text" onkeyup="searchItems()">
...
fetch("https://dummyjson.com/products")
    ...
    .then((res1) => {
        ...
        for (let x in res1.products) {
            ...
            document.getElementById(`img${u}`).src = res1.products[x].images[0];
            document.getElementById(`price${u}`).innerHTML = "Price : " + res1.products[x].price + "$";
            localStorage.setItem(`id${u}`, JSON.stringify(res1.products[x]));
        }
    });
...
function incrementValue(u) {
    ...
    if (number >= prodcutQuantity[u] || (JSON.parse(localStorage.getItem(`productquantity${u}`)) >= prodcutQuantity[u])) {
...
function incrementValuePannel(u) {
    ...
    let prodcutQuantity = JSON.parse(localStorage.getItem("prodcutQuantity"))
    let p = document.getElementById(`cardvaluepannel${u}`).innerHTML;
    if (p >= prodcutQuantity[u] || (JSON.parse(localStorage.getItem(`productquantity${u}`)) >= prodcutQuantity[u])) {
...
function searchItems() {
    let search_Items = document.getElementById("search-text").value;
    if (search_Items.length >= 3) {
        document.getElementById("cart-details").style.width = "600px";
        document.getElementById("maindiv3").innerHTML = " ";
       // let clear_value = searc_Items.match(/[0-9a-z]/gi).toString().split(",").join("")
        fetch(`https://dummyjson.com/products/search?q=${search_Items}`)
            .then((res) => {
                return res.json()
            })
            .then((res1) => {
                console.log(res1)
                for (let x of res1.products) {
                    if (x.id <= 30) {
                        // let product_title = (x.title);
                        // let b = product_title.toLowerCase()
                        // if(b.includes(searcItems)) {
                        let productId = Number(x.id - 1);
...
function searchProductIncrement(productId) {
    ...
    if (search_quantity >= prodcutQuantity[productId] || (JSON.parse(localStorage.getItem(`productquantity${productId}`)) >= prodcutQuantity[productId])) {
        alert(`${y.title} sold out from`);
        event.preventDefault();
    }
    else {
